...
:: Try (in order): WEBIDE_JDK, webstorm%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
...
SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.WebStorm2018.1\config\webstorm%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.WebStorm2018.1\config\webstorm%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.WebStorm2018.1\config\webstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, standard installation
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\webstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: default, universal package
  SET VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\webstorm%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_WEBIDE_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_WEBIDE.hprof" -Didea.paths.selector=WebStorm2018.1 %IDE_PROPERTIES_PROPERTY%
SET IDE_JVM_ARGS=-Didea.platform.prefix=WebStorm
